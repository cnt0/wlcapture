...
static uint8_t is_curr(void) { return thrd_equal(CURR, thrd_current()); }
...
static void capture_set_buffer_size(void *data, capture_session_t session,
...
static void capture_set_shm_format(void *data, capture_session_t session,
                                   enum wl_shm_format format) {
  struct capture_state *state = data;
  state->shm_format = format;
}

static void capture_set_dmabuf_format(void *data, capture_session_t session,
                                      uint32_t format,
...
static void capture_set_dmabuf_device(void *data, capture_session_t session,
                                      struct wl_array *devices) {
  struct capture_state *state = data;
  state->devices_cnt = 0;
  dev_t *cur;
  wl_array_for_each(cur, devices) {
    state->devices[state->devices_cnt++] = *cur;
  }
}
...
static void frame_transform(void *data, capture_frame_t frame,
                            uint32_t transform) {}

static void frame_damage(void *data, capture_frame_t frame, int32_t x,
                         int32_t y, int32_t width, int32_t height) {}

static void frame_presentation_time(void *data, capture_frame_t frame,
...
int main(int argc, char *argv[]) {
  ...
  struct ext_image_capture_source_v1 *src =
      capture_manager_create_source(wl_state.capture_manager, wl_state.output);
  ...
  AVDictionary *libjxl_opts = NULL;

  // av_dict_set(&libjxl_opts, "distance", "1.0", 0);
  // av_dict_set(&libjxl_opts, "effort", "9", 0);
  // av_dict_set(&libjxl_opts, "modular", "1", 0);
  // av_dict_set(&libjxl_opts, "xyb", "0", 0);
  ...
  src_frame->width = cp_state.width;
  src_frame->height = cp_state.height;
  src_frame->format = shm_buf.pixel_format;
  av_image_fill_arrays(src_frame->data, src_frame->linesize, shm_buf.mapping,
                       shm_buf.pixel_format, cp_state.width, cp_state.height,
                       4);

  AVFrame *final_frame = src_frame;

  SwsContext *sw_scale_ctx = NULL;
  if (shm_buf.pixel_format != av_tgt_fmt) {
    final_frame = av_frame_alloc();
    final_frame->width = cp_state.width;
    final_frame->height = cp_state.height;
    final_frame->format = av_tgt_fmt;
    av_frame_get_buffer(final_frame, 0);
    ...
    sws_scale(sw_scale_ctx, (const uint8_t *const *)src_frame->data,
              src_frame->linesize, 0, cp_state.height, final_frame->data,
              final_frame->linesize);
  }

  final_frame->color_range = ctx->color_range;
  final_frame->color_primaries = ctx->color_primaries;
  final_frame->color_trc = ctx->color_trc;
  final_frame->colorspace = ctx->colorspace;

  AVPacket *pkt = av_packet_alloc();
  if (!pkt) {
    return EXIT_FAILURE;
  }
  ret = avcodec_send_frame(ctx, final_frame);
  if (ret) {
    return EXIT_FAILURE;
  }
